                    ...
                    if (report_type == 1) //lay theo thoi gian ngay hien tai
                     {
                        ...
                        var dateend = DateTime.Now.Date.AddHours(23).AddMinutes(59).AddSeconds(59); 
                         ...
                         var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >= '"+datenow.ToString("yyyy-MM-dd HH:mm:ss")+"' AND Datebill <= '"+dateend.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
   ...
   var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >= '"+datenow.ToString("yyyy-MM-dd HH:mm:ss")+"' AND Datebill <= '"+dateend.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
                    ...
                    } else if (report_type == 2) // lay theo tuan
                    {
                                ...
                                DateTime ngayCuoiCung = ngayDauTien.AddDays(6);
                                    List<ReportMovieshow> reports = new List<ReportMovieshow>();
                                       ...
                                       var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >=  '"+ngayDauTien.ToString("yyyy-MM-dd HH:mm:ss")+"' AND Datebill <= '"+ngayCuoiCung.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
                    ...
                    } else if (report_type == 3) // lay theo thang
                   {
                                ...
                                List<ReportMovieshow> reports = new List<ReportMovieshow>();
                                     ...
                                     var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >=  '"+firstDayOfMonth.ToString("yyyy-MM-dd HH:mm:ss")+"' AND Datebill <=  '"+lastDayOfMonth.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
                   ...
                   } else if (report_type == 4) // lay theo 3 thang gan nhat
                    {
                                ...
                                List<ReportMovieshow> reports = new List<ReportMovieshow>();
                                      ...
                                      var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >=  '"+ngayDauThang1.ToString("yyyy-MM-dd HH:mm:ss")+"' AND Datebill <=  '"+ngayCuoiThang3.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
                   ...
                   } else if (report_type == 5) // lay theo nam
                   {
                    ...
                    DateTime ngayCuoiNam = new DateTime(ngayHienTai.Year, 12, 31);
                                ...
                                List<ReportMovieshow> reports = new List<ReportMovieshow>();
                                           ...
                                           var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >=  '"+ngayDauNam.ToString("yyyy-MM-dd HH:mm:ss")+"' AND Datebill <=  '"+ngayCuoiNam.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
        ...
        DateTime ngayCuoiNamTruoc = new DateTime(ngayHienTai.Year - 1, 12, 31);
                                ...
                                List<ReportMovieshow> reports = new List<ReportMovieshow>();
                                                                ...
                                                                var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >=  '"+ngayDauNamTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"'  AND Datebill <=  '"+ngayCuoiNamTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
        ...
        DateTime ngayCuoiNam3 = new DateTime(ngayHienTai.Year, 12, 31);
                                ...
                                List<ReportMovieshow> reports = new List<ReportMovieshow>();
                                          ...
                                          var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >=   '"+ngayDauNam1.ToString("yyyy-MM-dd HH:mm:ss")+"'  AND Datebill <=  '"+ngayCuoiNam3.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
        ...
        DateTime ngayCuoiThangTruoc = new DateTime(ngayHienTai.Year, ngayHienTai.Month, 1).AddDays(-1);
                                ...
                                List<ReportMovieshow> reports = new List<ReportMovieshow>();
                                         ...
                                         var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY IFNULL(SUM(b.Totalamount), 0) DESC) as stt,m.Idmovie, m.poster,IFNULL(SUM(b.Totalamount), 0) AS totals,DATE_FORMAT(MAX(b.Datebill), '%Y-%m-%d %H') as Datebill FROM cinema.movie AS m LEFT JOIN (SELECT Idmovie,Totalamount,Datebill FROM cinema.Bill WHERE  Datebill >=   '"+ngayDauThangTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"' AND Datebill <=  '"+ngayDauThangTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"') AS b ON m.Idmovie = b.Idmovie GROUP BY m.Idmovie, m.poster HAVING Datebill IS NOT NULL AND totals != 0 ORDER BY totals DESC LIMIT 5;";
...
public IActionResult ReportFoods(int report_type)
{
    ...
    //check header
               ...
               try
                 {
                    ...
                    if (report_type == 1) {
                        ...
                        var dateend = DateTime.Now.Date.AddHours(23).AddMinutes(59).AddSeconds(59); 
                         ...
                         var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >= '"+datenow.ToString("yyyy-MM-dd HH:mm:ss")+"' AND pay.datetimes <= '"+dateend.ToString("yyyy-MM-dd HH:mm:ss")+"' GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
                        ...
                        //   }
                         ...
                         List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                              var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >= '"+ngayDauTien.ToString("yyyy-MM-dd HH:mm:ss")+"' AND pay.datetimes <= '"+ngayCuoiCung.ToString("yyyy-MM-dd HH:mm:ss")+"' GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
                        ...
                        //   }
                         ...
                         List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                            var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >=  '"+firstDayOfMonth.ToString("yyyy-MM-dd HH:mm:ss")+"' AND pay.datetimes <= '"+lastDayOfMonth.ToString("yyyy-MM-dd HH:mm:ss")+"' GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
                        ...
                        successApiResponse.Data = dataresponse;
                         ...
                         List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                            var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >=  '"+ngayDauThang1.ToString("yyyy-MM-dd HH:mm:ss")+"' AND pay.datetimes <= '"+ngayCuoiThang3.ToString("yyyy-MM-dd HH:mm:ss")+"' GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
                    ...
                    DateTime ngayCuoiNam = new DateTime(ngayHienTai.Year, 12, 31);
                         ...
                         List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                            var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >=  '"+ngayDauNam.ToString("yyyy-MM-dd HH:mm:ss")+"'  AND pay.datetimes <= '"+ngayCuoiNam.ToString("yyyy-MM-dd HH:mm:ss")+"'  GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
                        ...
                        DateTime ngayCuoiNamTruoc = new DateTime(ngayHienTai.Year - 1, 12, 31);
                         List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                            var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >=  '"+ngayDauNamTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"'  AND pay.datetimes <= '"+ngayCuoiNamTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"'  GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
        ...
        DateTime ngayCuoiNam3 = new DateTime(ngayHienTai.Year, 12, 31);
                          List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                             var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >= '"+ngayDauNam1.ToString("yyyy-MM-dd HH:mm:ss")+"' AND pay.datetimes <= '"+ngayCuoiNam3.ToString("yyyy-MM-dd HH:mm:ss")+"'   GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
        ...
        DateTime ngayCuoiThangTruoc = new DateTime(ngayHienTai.Year, ngayHienTai.Month, 1).AddDays(-1);
                          List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                            var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood WHERE  pay.datetimes >= '"+ngayDauThangTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"'  AND pay.datetimes <= '"+ngayCuoiThangTruoc.ToString("yyyy-MM-dd HH:mm:ss")+"'   GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
                        ...
                        //   }
                          ...
                          List<ReportFood> reports = new List<ReportFood>();
                         var sql = "";
                                var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,fd.Idfood,combo.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.listFoodCombo cb ON pay.id = cb.idfoodcombobill INNER JOIN cinema.FoodComboBill combo on combo.idcombo = cb.Idfoodcombo INNER JOIN cinema.food fd on fd.Idfood = combo.Idfood  GROUP BY fd.Idfood, combo.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
                        // var sqltest = "SELECT ROW_NUMBER() OVER (ORDER BY MAX(pay.datetimes) DESC) as stt,cb.Idfood,cb.idcombo, IFNULL(SUM(pay.total_price), 0) as totals, MAX(pay.datetimes) as datetimes FROM  cinema.FoodCombillPayment pay INNER JOIN cinema.FoodComboBill cb ON pay.idFoodlistcombo = cb.Id GROUP BY cb.Idfood, cb.idcombo ORDER BY MAX(pay.datetimes) DESC LIMIT 0, 1000";
